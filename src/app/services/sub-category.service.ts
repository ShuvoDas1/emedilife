...
export class SubCategoryService {
  ...
  getAllSubCategoryWithCount(pagination?: Pagination) {
    if (pagination) {
      let params = new HttpParams();
      params = params.append('pageSize', pagination.pageSize);
      params = params.append('page', pagination.currentPage);
      return this.httpClient.get<{ data: ProductSubCategory[], message?: string, count: number }>(API_SUB_CATEGORY + 'get-all-sub-categories-with-count', { params });
    } else {
      return this.httpClient.get<{ data: ProductSubCategory[], message?: string, count: number }>(API_SUB_CATEGORY + 'get-all-sub-categories-with-count');
    }

...
export class ProductsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private subBrand: Subscription;
  private subGeneric: Subscription;
  ...
  stockTypes: Select[] = [
    {value: true, viewValue: 'Stock In'},
    {value: false, viewValue: 'Stock Out'},
  ...
  @ViewChild('matStockSelect') matStockSelect: MatSelect;
  @ViewChild('matBrandSelect') matBrandSelect: MatSelect;
  @ViewChild('matGenericSelect') matGenericSelect: MatSelect;
  ...
  private getAllProducts() {
    ...
    const select = 'productName productSlug sku images category subCategory generic prices stockVisibility';
    const sort = {createdAt: -1};
    this.subProduct = this.productService.getProductList(pagination, sort, this.query, select)
      .subscribe(res => {
        this.spinner.hide();
  ...
  private getAllBrands() {
    this.subBrand = this.brandService.getAllBrands()
      .subscribe(res => {
  ...
  private getAllGenerics() {
    this.subGeneric = this.genericService.getAllGenerics()
      .subscribe(res => {
  ...
  onSelectBrand(event: MatOptionSelectionChange) {
    if (event.isUserInput) {
      const brand = event.source.value as ProductBrand;
      this.query = {brand: brand._id};
      if (this.currentPage > 1) {
        this.router.navigate([], {queryParams: {page: 1}});
      } else {
        this.getAllProducts();
  ...
  onSelectGeneric(event: MatOptionSelectionChange) {
    if (event.isUserInput) {
      const generic = event.source.value as ProductGeneric;
      this.query = {generic: generic._id};
      if (this.currentPage > 1) {
        this.router.navigate([], {queryParams: {page: 1}});
      } else {
        this.getAllProducts();
  ...
  onSelectStockType(event: MatOptionSelectionChange) {
    if (event.isUserInput) {
      this.query = {...this.query, ...{stockVisibility: event.source.value}};
      if (this.currentPage > 1) {
        this.router.navigate([], {queryParams: {page: 1}});
      } else {
        this.getAllProducts();
  ...
  onClearFilter() {
    ...
    this.matStockSelect.options.forEach((data: MatOption) => data.deselect());
    this.matBrandSelect.options.forEach((data: MatOption) => data.deselect());
    this.matGenericSelect.options.forEach((data: MatOption) => data.deselect());

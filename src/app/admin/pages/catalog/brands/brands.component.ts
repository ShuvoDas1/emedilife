...
export class BrandsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private getAllBrands() {
    ...
    this.brandService.getAllBrandsWithCount(pagination)
      .subscribe(res => {
  ...
  exportExcelFormat() {
    ...
    const data: ProductBrand[] = [
      { brandName: 'Test Brand 22', image: null, phoneNo: null, email: null, address: null }
    ];
    // EXPORT XLSX
    const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(data);
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'brands');
    XLSX.writeFile(wb, `Brands_Import_Format.xlsx`);
    this.spinner.hide();
  }
